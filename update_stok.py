...
# Menggunakan link gambar yang valid agar bisa didownload bot
...
async def cek_stok():
    async with async_playwright() as p:
        ...
        try:
            ...
            print(f"Mengirim Update ke Discord dengan Foto...")

            if WEBHOOK_URL:
                # 1. Download gambar ke folder bot sementara agar tidak pecah/mati
                img_data = requests.get(URL_FOTO_PRODUK).content
                with open('produk.png', 'wb') as handler:
                    handler.write(img_data)

                # 2. Siapkan Payload (Pesan & Struktur Box)
                payload = {
                    ...
                    "embeds": [{
                        ...
                        "color": warna_embed,
                        "image": {"url": "attachment://produk.png"}, # Memanggil file yang diupload
                        "footer": {"text": "Bot Monitor Itemku • Status: Active Always"}
                    }]
                }

                # 3. Kirim File Gambar + Payload JSON secara bersamaan
                ...
                if response.status_code == 204 or response.status_code == 200:
                    print("Update Berhasil Terkirim!")
                else:
